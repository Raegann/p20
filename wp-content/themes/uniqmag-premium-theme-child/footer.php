<footer>
    <div class="cs-container">
            <div class="cs-row">
                <img class="pattern" src="<?php bloginfo('stylesheet_directory');?>/images/paticka_obili.png" alt="logo">
		  <div class="cs-col cs-col-4-of-12">
                      <img class="logo" src="<?php bloginfo('stylesheet_directory');?>/images/erb.png" alt="erb">
			<ul class="footer_menu">
				<li><a href="">Mapa webu</a></li>
				<li><a href="">Prohlášení o přístupnosti webu</a></li>
			</ul>
			  <p class="copyright">Copyright 2016 ÚMČ Praha 20</p>
 
		  </div>
		  <div class="center cs-col cs-col-4-of-12">
		
			  <h3>Úřední hodiny</h3>
			  <p>Pondělí, středa: 7:30 - 12:00 a 13:00 - 18:00<br>Pátek: 7:30 - 11:00</p>
			  <p>Úřad městské části Praha 20<br>Jívanská 647, 193 21 Praha 9</p>
                          <p>Tel: 271 071 611, Email: <a href="mailto:urad@pocernice.cz">urad@pocernice.cz</a></p>
			  <p>Datová schránka: seibq29</p>
		  
		  </div>
		  <div class="right cs-col cs-col-4-of-12">
			  <div class="clear"></div>
			  <img class="footer_logo" src="<?php bloginfo('stylesheet_directory');?>/images/footerlogo.png" alt="logo">
			  
            </div>
	</div>
      </div>
</footer>
	<?php wp_footer(); ?>
	<!-- END body -->
	</body>
<!-- END html -->
</html>