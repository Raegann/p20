	...
	echo '<div class="foovideo-result foovideo-youtube">';
		...
		echo '<div class="foovideo-details">';
			...
			echo '<div class="foovideo-meta">';
				if( empty( $isplaylist ) ){
					echo __('by', 'foo-video' ) . ' ' . $video['author'] . '&bull; ';
				}
				echo sprintf( _x( '%s ago', '%s = human-readable time difference', 'foo-video' ), human_time_diff( $video['time_created'], current_time( 'timestamp' ) ) );
				echo ' &bull; ' . sprintf( _n( '%s view', '%s views', $video['views'], 'foo-video' ), $video['views'] );
			echo '</div>';
			echo '<div class="foovideo-description">';
				echo substr( $video['description'],0, 280 ) . '&hellip;';
			echo '</div>';
			echo '<input type="hidden" class="foovideo-import" value="' . foogallery_video_esc_json_encode( $video ) . '">';
		echo '</div>';
	echo '</div>';
