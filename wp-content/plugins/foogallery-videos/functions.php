 ...
 * @return array
 */
function foo_video_import_video_as_attachment( $video_object ) {

	$thumbnail = $video_object['thumbnail'];
	$attachment_id = foogallery_video_get_attachment_id_by_url( $thumbnail );
	...
	update_post_meta( $attachment_id, '_foogallery_custom_target', '_blank' );

	//save video object
	update_post_meta( $attachment_id, FOO_VIDEO_POST_META, $video_object );
 ...
 * @return int
 */
function foogallery_video_get_gallery_video_count( $post_id ) {
	$video_count = get_post_meta( $post_id , FOO_VIDEO_POST_META_VIDEO_COUNT, true );
 ...
 * @param $post_id
 */
function foogallery_video_set_gallery_video_count( $post_id ) {
	...
	if ( $video_count > 0 ) {
		...
		delete_post_meta( $post_id, FOO_VIDEO_POST_META_VIDEO_COUNT );
	}
}
 ...
 * @return int
 */
function foogallery_video_calculate_gallery_video_count( $post_id ) {
	$video_count = 0;
 ...
 * @return string|void
 */
function foogallery_video_gallery_image_count_text( $total_count, $image_count, $video_count ) {
 ...
 * @return string
 */
function foogallery_video_esc_json_encode( $data ) {
...
/**
 * Returns if the FooVideo plugin can run. Checks if FooGallery is installed and is at the minimum version
 *
 * @return bool
 */
function foogallery_video_plugin_can_run() {
	return class_exists( 'FooGallery' ) && version_compare( FOOGALLERY_VERSION, FOO_VIDEO_FOOGALLERY_MIN_VERSION ) >= 0;
}

/**
 * Checks to see if the FooVideo plugin is already running
 */
function foogallery_video_plugin_already_initialized() {
	global $foogallery_extensions;
	if ( $foogallery_extensions ) {
		foreach ( $foogallery_extensions as $foogallery_extension ) {
			if ( 'Foo_Video' == get_class( $foogallery_extension ) ) {
				return true;
			}
		}
	}
 ...
 */
function foogallery_video_clean_video_url( $url ) {
	//make the video URL Protocol-relative to cater for sites that are HTTPS
	$url = str_replace( 'http://', '//', $url );

	return apply_filters( 'foogallery_video_clean_video_url', $url );
}
 ...
 */
function foogallery_video_get_video_url_from_attachment( $attachment ) {
	...
	return foogallery_video_clean_video_url( $url );
}

function foogallery_video_get_video_thumbnail_from_attachment( $attachment ) {
	$args = array(
		'width'  => 90,
		'height' => 90,
		'crop'   => true
 ...
 * @return mixed|void
 */
function foogallery_video_youtubekey() {
	return apply_filters( 'foogallery_video_youtubekey', 'AIzaSyBMT07ftYs1dGnguTdI8I_fXazRyrnZcEA' );
}
 ...
 */
function foogallery_video_get_attachment_id_by_url($url) {
	global $wpdb;
