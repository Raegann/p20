<?php
if ( ! class_exists( 'Foo_Video_Import_Handler_YouTube' ) ) {
	class Foo_Video_Import_Handler_YouTube {
		...
		protected function do_import( $playlist_id, $offset ) {
			...
			if ( is_array( $feed ) && !empty( $feed['video'] ) ) {
				...
				foreach( $feed['video'] as $index => $video ) {
					...
					$attachment = foo_video_import_video_as_attachment( $item );
					$attachments['ids'][] = $attachment;

					if( count( $attachments['ids'] ) >= FOO_VIDEO_BATCH_LIMIT && ( $total - 1 ) > $index ){
						$part = true;
						break;
					}
				}
